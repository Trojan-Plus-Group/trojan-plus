...
class DNSServer : public std::enable_shared_from_this<DNSServer>{
    ...
    enum{
        DEFAULT_UP_STREAM_NS_SVR_PORT = 53
    };
    ...
    public:
        ...
        [[nodiscard]] inline bool expired(time_t curr) const{
            return int(curr - cached_time) >= ttl;
        }
    ...
    [[nodiscard]] 
    bool is_in_gfwlist(const std::string& domain)const;

    [[nodiscard]]
    bool try_to_find_existed(const boost::asio::ip::udp::endpoint &local_src, const std::string_view& data);

    [[nodiscard]]
    static bool is_proxy_dns_msg(const trojan::dns_header& hdr);

    [[nodiscard]]
    static bool is_proxy_dns_msg(const trojan::dns_question& question);
