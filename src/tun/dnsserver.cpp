...
bool DNSServer::get_dns_lock() {
    s_dns_file_lock = get_file_lock("./trojan_dns_lock");
    return s_dns_file_lock != -1;
}

DNSServer::DNSServer(Service* _service): 
 m_service(_service),
 m_serv_udp_socket(_service->get_io_context()){  
}
...
bool DNSServer::is_proxy_dns_msg(const trojan::dns_question& qt){
    return qt.get_QCLASS() == dns_header::QCLASS_INTERNET 
        && (qt.get_QTYPE() == dns_header::QTYPE_A_RECORD
        || qt.get_QTYPE() == dns_header::QTYPE_AAAA_RECORD);
}
...
bool DNSServer::find_in_dns_cache(const udp::endpoint &local_src, const dns_header& header, const dns_question& question){
    if(m_service->get_config().get_dns().enable_cached){
        ...
        for(auto& c : m_dns_cache){
            if(c.get_domain() == question.get_QNAME()){
                _log_with_endpoint_ALL(local_src, "[dns] find " + question.get_QNAME() + 
                    " in cache ttl: "+ to_string(c.get_ttl() - (curr_time - c.get_cached_time())));
...
void DNSServer::store_in_dns_cache(const string_view& data){
    if(m_service->get_config().get_dns().enable_cached){
        ...
        dns_answer answer;
        if(is >> answer){
            if(!answer.get_questions().empty() 
              && is_proxy_dns_msg(answer.get_questions()[0])){
...
void DNSServer::in_recved(istream& is, const dns_header& header, const string_view& former_data){
    ...
    if(proxy){
        ...
        auto forwarder = make_shared<UDPForwardSession>(m_service, m_service->get_config(), m_service->get_ssl_context(), 
        m_udp_recv_endpoint, dst, [this](const udp::endpoint &endpoint, const string_view &data) {
            recv_up_stream_data(endpoint, data);
        }, false, true);
        ...
        auto forwarder = make_shared<UDPLocalForwarder>(m_service, m_udp_recv_endpoint, dst, 
